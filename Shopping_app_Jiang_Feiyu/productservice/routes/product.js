...
router.get("/loadpage", (req, res) => {
  ...
  if (category == "") {
    ...
    // 若搜索内容不为空，则返回对应内容
      ...
      col.find({ name: { $regex: searchstring } }, {}).then((docs) => {
        console.log("dew");
        res.json(docs);
      });
    }
      ...
      if (user[0] != null) {
        ...
        for (var i = 0; i < user[0].cart.length; i++) {
          if (user[0].cart[i].productId == productId) {
            user[0].cart[i].quantity =
              user[0].cart[i].quantity + Number(quantity);
        ...
        userC.update(
          ...
          { $set: { cart: user[0].cart, totalnum: totalnum } },
          (err, result) => {}
        );
...
router.get("/loadcart", (req, res) => {
  ...
  userC
    ...
    .then((user) => {
      ...
      col.find({ _id: { $in: mycartproducts } }).then((products) => {
        console.log("cded");
        ...
        for (var i = 0; i < products.length; i++) {
          ...
          mycarts[i] = {
            ...
            quantity: quantity,
            images: products[i].productImage,
          };
        }
        console.log(mycarts);
        res.send({
...
//Task1.9 删除商品 （未完成）
router.post("/deletefromcart/:productid", (req, res) => {
  // 参数提取
