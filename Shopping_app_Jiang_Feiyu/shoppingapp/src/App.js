...
class App extends React.Component {
  ...
  home() {
    ...
    return (
      <div>
        <ul className="products">{this.listItems}</ul>
      </div>
    );
  }
  render() {
  ...
  render() {
    return (
      <div>
        ...
        <p>$XX paid</p>
        <Link to="/home">
          <button className="continue-browsing">continue browsing &gt; </button>{" "}
        </Link>
      </div>
    );
  }
...
class Cart extends React.Component {
  ...
  handlerChange(e, index) {
    ...
    //更改的数量
    let value = Number(e.target.value < 0 ? 0 : e.target.value);
    console.log(value);
    ...
    // console.log(this.props.arr[index]);
    this.setState({ numbers: [...arr] });
    // console.log(this.props.arr[index]);
    var id = arr[index].productId;
    console.log(id);
    if (Number(value) === 0) {
      $.ajax({
        type: "POST",
        url: "http://localhost:3001/deletefromcart/" + id,
        dataType: "json",
        crossDomain: true, //设置跨域为true
        xhrFields: {
          withCredentials: true, //默认情况下，标准的跨域请求是不会发送cookie的
  ...
  render() {
    ...
    //console.log(numbers.items.images)
    return (
      <div>
            ...
            {this.state.numbers.map((item, index) => (
              <tr key={item.productId}>
                <td className="detail-img">
                  <img
                    src={`http://127.0.0.1:3001/` + `${item.images}`}
                    alt=""
                  />
                </td>
        ...
        <p>
          Cart subtotal ({this.state.numbers.length} items) : ${totalPrice}
        </p>
  ...
  render() {
    return (
      <button onClick={this.goBack} className="back-btn">
        <img src="http://127.0.0.1:3001/images/back.png"></img>go back
      </button>
    );
  }
}
    ...
    if (this.state.toCartPage) {
      return (
        <div>
              ...
              <tr>
                <td className="detail-img">
                  ...
                  <img
                    src={
                      `http://127.0.0.1:3001/` +
                      `${this.state.detailInfo.productImage}`
                    }
                    alt=""
                  />
                </td>
          ...
          <Link to="/home">
            <button className="continue-browsing">
              continue browsing &gt;{" "}
            </button>{" "}
          </Link>
        </div>
      ...
      <div className="login-page">
        <div className="login-box">
          ...
          <div>
            ...
            <input type="password" onChange={this.handlerInput} />
          </div>
...
class Header extends React.Component {
  ...
  logOut() {
    $.ajax({
      ...
      async: false,
    }).done(function (response) {}.bind(this));
    // 刷新页面返回首页
    window.location.href = "/#/home";
    window.location.reload();
  }
  ...
  render() {
    ...
    if (userId) {
      button = (
        <div>
          ...
          <div>
            <div onClick={this.logOut} className="login-out">
              Sign out
            </div>
          </div>
        </div>
    ...
    return (
      <header>
        ...
        <div className="login-name">{button}</div>
        <div className="search">
          ...
          <input type="text" onChange={this.handlerInput} />
          <button onClick={this.search} className="search-icon">
            <img src="http://127.0.0.1:3001/images/search.png" />
          </button>
        </div>
        <div className="cart-info">{cartInfo}</div>
      </header>
    );
  }
}
